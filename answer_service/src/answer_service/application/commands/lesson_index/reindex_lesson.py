...
class ReindexLessonCommandHandler:
    ...
    async def __call__(self, data: ReindexLessonCommand) -> None:
        log = logger.bind(lesson_id=str(data.lesson_id))
        log.info("reindex_lesson: started")
        ...
        log.debug("reindex_lesson: text split", chunks_count=len(chunk_contents))
        ...
        log.info("reindex_lesson: done", chunks_indexed=len(chunk_contents))
